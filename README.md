...
## :one: 由来
新建项目后，经常发现连个`string.IsEmpty()`都没有 :disappointed_relieved: 、来个`Json`的通用返回值吧，也没有:unamused: 还有等等等等等等等等。。。所以就决定整理并集成一下自己平时用的比较多的一些工具类，然后发布到`NuGet`，以后走到哪用到哪有木有:dog::dog:  
